  ...
  return (
    <>
      <div className="w-5/6 mx-auto flex h-[50px] items-center justify-between">
        <img src={Logo}></img>
        <button className="w-[120px] h-[40px] hover:bg-violet-400 duration-300 flex items-center justify-between p-5 bg-violet-100 text-[14px] font-semibold text-violet-600 rounded-md"><Layers className="mr-[5px]"/> Каталог</button>
        <input
          type="text"
          name=""
          id=""
       ...
       {navigation.map((item, idx) => (
        <div key={idx} className="flex items-center cursor-pointer">
          {item.img} 
          <p className="text-[14px] hover:font-semibold duration-300 ml-[15px]">{item.text}</p>
        </div>
       ))}
      </div>
    </>
